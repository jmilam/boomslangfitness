class MessageController < ApplicationController
  ...
  def new
    ...
    gym_admin = GymAdmin.find_by(gym_id: gym.id, user_id: current_user.id)
    @clients = if gym_admin
      User.where(gym_id: gym.id).not_self(current_user.id)
    else
      User.where(trainer_id: current_user.id, gym_id: gym.id).not_self(current_user.id)
    end
  end
